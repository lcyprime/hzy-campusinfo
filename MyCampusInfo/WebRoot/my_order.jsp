	...
	<body>
		...
		<form action="cancelOrder.action" id="cancelOrder" method="post">
			<table id="mytable" cellspacing="0">
				...
				<c:forEach items="${myOrder}" var="order" varStatus="status">
					<tr>
						<td class="row">
							<input type="checkbox" name="business_name" value="${order.orderBusiness}"
								id="${status.count}">
						</td>
						<td class="row">
							&nbsp;${status.count}&nbsp;
						...
						<td class="row">
							<c:if test="${order.orderState == 1}">已订阅</c:if>
							<c:if test="${order.orderState == 0}">已退订</c:if>
						</td>
					</tr>
				</c:forEach>
			</table>
